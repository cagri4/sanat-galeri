...
export default async function InstagramSection({
  ...
  return (
    <section className="border-t border-[#e9e7e1] py-20">
      <div className="text-center">
        <p className="text-[length:var(--text-label)] uppercase tracking-[var(--tracking-label)] text-[#999]">
          Instagram
        </p>
        <h2 className="mt-3 text-3xl font-medium text-[#2C2C2C] sm:text-4xl">
          {t('title')}
        </h2>
      ...
      <div className="mt-12 grid grid-cols-3 gap-2 sm:gap-3">
        {items.map((it, i) => (
          <a
            ...
            className="media-zoom group relative aspect-square overflow-hidden bg-[#f0efe9]"
          >
            <Image
              src={it.src}
              alt={it.alt}
      ...
      <div className="mt-10 text-center">
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="group inline-flex min-h-11 items-center gap-2.5 border border-[#2C2C2C] px-6 py-3 text-[length:var(--text-meta)] uppercase tracking-[var(--tracking-label)] text-[#2C2C2C] transition-colors duration-[var(--dur-micro)] hover:border-[var(--accent)] hover:bg-[var(--accent)] hover:text-white"
        >
